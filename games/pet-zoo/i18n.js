...
// What sits between the two numbers when the question is read out loud. "ganger" rather than
// "multiplisert med" for the same reason the rest of this file says "pluss": it is the word a
// child and a grown-up sitting beside them actually use.
const JOINERS = {
  en: { '+': 'plus', '-': 'minus', '×': 'times', '÷': 'divided by' },
  nb: { '+': 'pluss', '-': 'minus', '×': 'ganger', '÷': 'delt på' },
};

/** And any of the ways this game writes a question down. */
export const spokenFact = (lang, { op = '+', a, b, gap = false } = {}) => {
  const code = NUMBER_WORDS[lang] ? lang : DEFAULT_LANGUAGE;
  const joiners = JOINERS[code] ?? JOINERS[DEFAULT_LANGUAGE];
  const joiner = joiners[op] ?? joiners['+'];
  // A missing factor is the only question whose blank is not at the end, and the aria label is
  // all a screen reader gets — so it is read as the question it is, with the gap spoken aloud.
  if (gap) {
    const hole = code === 'en' ? 'what' : 'hva';
    const makes = code === 'en' ? 'makes' : 'blir';
    return `${numberWord(code, a)} ${joiner} ${hole} ${makes} ${numberWord(code, a * b)}`;
  }
...
export const NAMES = {
  en: [
    ...
    'Snowdrop',
    // Appended when the ladder outgrew the forty-eight above. Only ever appended — see
    // `nameFrom` in pets.js for why the first forty-eight can never be reordered or widened.
    'Barnacle', 'Chestnut', 'Dumpling', 'Flapjack', 'Gooseberry', 'Hiccup', 'Juniper',
    'Kipper', 'Lollipop', 'Nettle', 'Parsnip', 'Quince', 'Rhubarb', 'Scribble', 'Thimble',
    'Waffle-cone',
  ],
  nb: [
    ...
    // Appended when the ladder outgrew the forty-eight above. Only ever appended — see
    // `nameFrom` in pets.js for why the first forty-eight can never be reordered or widened.
    'Sukkerspinn', 'Havregryn', 'Skogsbær', 'Einer', 'Brisen', 'Skodda', 'Rimfrost',
    'Barken', 'Siv', 'Tangen', 'Hikke', 'Klatten', 'Snurra', 'Vippa', 'Loffen', 'Kruset',
  ],
};
...
const STRINGS = {
  en: {
    ...
    // The only place the weather is put into words. The scene itself stays wordless.
    'habitat.ariaWeather': '{home}, {weather}',
    'weather.clear': 'sunny',
    ...
    'tier.math.19.name': 'Ones and tens',
    'tier.math.19.blurb': 'Nothing changes, or a zero goes on the end.',
    'tier.math.20.name': 'Twos',
    'tier.math.20.blurb': 'Doubling, which you already know from adding.',
    'tier.math.21.name': 'Fives',
    'tier.math.21.blurb': 'Half of ten, every time.',
    'tier.math.22.name': 'Squares',
    'tier.math.22.blurb': 'A number times itself: three threes, four fours.',
    'tier.math.23.name': 'Threes and fours',
    'tier.math.23.blurb': 'Count in threes, or double and double again.',
    'tier.math.24.name': 'The hard six',
    'tier.math.24.blurb': 'Six, seven, eight and nine — the last ones left.',
    'tier.math.25.name': 'Missing ones and tens',
    'tier.math.25.blurb': 'The same two rules, asked the other way round.',
    'tier.math.26.name': 'Missing twos',
    'tier.math.26.blurb': 'How many twos does it take to get there?',
    'tier.math.27.name': 'Missing fives',
    'tier.math.27.blurb': 'Count up in fives until you land on it.',
    'tier.math.28.name': 'Missing squares',
    'tier.math.28.blurb': 'Which number, times itself, makes this one?',
    'tier.math.29.name': 'Missing threes and fours',
    'tier.math.29.blurb': 'You know the product — find the number that made it.',
    'tier.math.30.name': 'The hard six, missing',
    'tier.math.30.blurb': 'The six you found hardest, hunted backwards.',
    'group.plus': 'Adding',
    'group.minus': 'Taking away',
    'group.tens': 'Tens and ones',
    'group.column': 'Columns',
    'group.times': 'Multiplying',
    'tier.math.31.name': 'Multiplying by tens',
    'tier.math.31.blurb': 'The digits stay put and a zero goes on the end.',
    'tier.math.32.name': 'Stacked, nothing to carry',
    'tier.math.32.blurb': 'Line them up and multiply each column on its own.',
    'tier.math.33.name': 'Stacked, with carrying',
    'tier.math.33.blurb': 'Multiply first, then add what was carried.',
    'tier.math.34.name': 'Three digits, stacked',
    'tier.math.34.blurb': 'Carrying twice, all the way along.',
    'tier.math.35.name': 'Two rows to add up',
    'tier.math.35.blurb': 'Multiply by the ones, then by the tens, then add.',
    'tier.math.36.name': 'Three digits by two',
    'tier.math.36.blurb': 'The whole method, from end to end.',
    'group.gap': 'The missing number',
    'group.colmul': 'Multiplying in columns',
    'skill.tensx': 'Times a whole ten',
    'skill.colx21': 'Stacked ×',
    'skill.colx21c': 'Stacked × carry',
    'skill.colx31c': 'Three-digit ×',
    'skill.colx22': 'Two rows',
    'skill.colx32': 'Three by two',
    // Which sign a division is written with is a fact about where a child goes to school, not
    // about arithmetic. Norwegian classrooms use the colon; English-speaking ones use the
    // obelus. The id in a save stays `div:` either way — see divfacts.js.
    'sign.divide': '÷',
    'tier.math.37.name': 'Sharing by one and ten',
    'tier.math.37.blurb': 'Nothing changes, or a zero comes off the end.',
    'tier.math.38.name': 'Sharing in half',
    'tier.math.38.blurb': 'Halving, which you already have from doubling.',
    'tier.math.39.name': 'Sharing by five',
    'tier.math.39.blurb': 'Half of ten, counted back in fives.',
    'tier.math.40.name': 'The squares, shared back',
    'tier.math.40.blurb': 'Forty-nine into sevens, and the rest of them.',
    'tier.math.41.name': 'Sharing by three and four',
    'tier.math.41.blurb': 'The tables you built up in threes and fours.',
    'tier.math.42.name': 'The last six',
    'tier.math.42.blurb': 'The ones every child finds hard.',
    'tier.math.43.name': 'Dividing by a whole ten',
    'tier.math.43.blurb': 'The digits stay put and a zero comes off.',
    'tier.math.44.name': 'Setting out a division',
    'tier.math.44.blurb': 'One digit at a time, from the biggest end.',
    'tier.math.45.name': 'Taking the leftover down',
    'tier.math.45.blurb': 'What is left joins the next digit along.',
    'tier.math.46.name': 'When the first digit is too small',
    'tier.math.46.blurb': 'Take two digits together, and the answer comes out shorter.',
    'tier.math.47.name': 'Three digits, shared out',
    'tier.math.47.blurb': 'The same three steps, one more time.',
    'tier.math.48.name': 'A nothing in the answer',
    'tier.math.48.blurb': 'The step comes to nothing, and the nothing still gets written.',
    'tier.math.49.name': 'Something left over',
    'tier.math.49.blurb': 'Divide until nothing more comes down.',
    'tier.math.50.name': 'The whole method',
    'tier.math.50.blurb': 'Three digits, and whatever is left at the end.',
    'group.divfacts': 'Sharing out',
    'group.coldiv': 'Long division',
    'skill.div10': 'Divide by a whole ten',
    'skill.div21': 'Set out ÷',
    'skill.div21x': 'Take the leftover down',
    'skill.div21s': 'A shorter answer',
    'skill.div31': 'Three-digit ÷',
    'skill.div31z': 'A nothing in the answer',
    'skill.div21r': 'Something left over',
    'skill.div31r': 'Three digits with a leftover',
    'teach.divGaveDividend': 'That is the number you were sharing out.',
    'teach.divGaveDivisor': 'That is how many you were sharing it between.',
    'teach.divTookAway': 'Not taken away — how many of them fit inside.',
    'teach.divMultiplied': 'That is them multiplied, not shared out.',
    'teach.divNeighbour': 'That is in the right table, but it is a different one of them.',
    'teach.divPlain': '{b} goes into {a} {answer} times.',
    'teach.divFamily': 'You know {small} × {large} = {a}, so {a} ÷ {b} = {answer}.',
    'teach.divForgotBringDown': 'Bring the next digit down beside what is left.',
    'teach.divIgnoredRemainder': 'Something was left over, and it does not just go away.',
    'teach.divZeroStep': 'It does not go in there at all — write a nothing and carry on.',
    'teach.divDroppedRemainder': 'What is left over goes down to join the next digit.',
    'teach.divBackwards': 'The number on top is the one being shared out, every time.',
    'teach.divMulInstead': 'That is them multiplied, and this one is shared out.',
    'teach.divSubInstead': 'That is them taken away, and this one is shared out.',
    'teach.divColPlain': '{a} shared between {b} is {quotient}.',
    'teach.divColRest': '{a} shared between {b} is {quotient}, with {rest} left over.',
    'prompt.divEgg': 'A chilly egg! Set it out and share it:',
    'prompt.divEgg1': 'The egg is stirring! Step by step:',
    'prompt.divEgg2': 'It is cracking open! One more to share:',
    'prompt.divForgot': '{name} forgot their snack. Share it out:',
    'prompt.divHungry': '{name} is hungry! Their snack is this sum:',
    'prompt.divSnack': '{name} fancies a snack. Share it out:',
    'teach.mulColForgotShift': 'That row is the tens, so it needs its zero on the end.',
    'teach.mulColCarriedFirst': 'Multiply the digit first, and add the carry to what it comes to.',
    'teach.mulColFullProduct': 'A column only keeps one digit — the rest carries.',
    'teach.mulColForgotCarry': 'What ran over the ten has to go next door.',
    'teach.mulColOnlyOnes': 'That is only the first row — the tens still have their turn.',
    'teach.mulColAddedInstead': 'That is them added together, not multiplied.',
    'teach.mulColPlain': '{a} times {b} makes {total}.',
    ...
    'prompt.gapEgg': 'A chilly egg! Which number is missing?',
    'prompt.gapEgg1': 'The egg is stirring! Find the missing one:',
    'prompt.gapEgg2': 'It is cracking open! One more missing number:',
    'prompt.gapForgot': '{name} forgot their snack. Which number is missing?',
    'prompt.gapHungry': '{name} is hungry! Find the missing number:',
    'prompt.gapSnack': '{name} fancies a snack. Which number is missing?',
    'teach.mulGaveSum': 'That is them added together, not one of them lots of times.',
    'teach.mulGaveFactor': 'That is one of the numbers on its own.',
    'teach.mulOffByOneRow': 'One row too many, or one too few — count the rows again.',
    'teach.mulNeighbour': 'That is in the right table, but it is a different one of them.',
    'teach.mulPlain': '{a} lots of {b} makes {product}.',
    'teach.gapGaveProduct': 'That is where you are going, not the number that gets you there.',
    'teach.gapGaveFactor': 'That is the number you were given already.',
    'teach.gapTookAway': 'That is taking it away — this one asks how many lots of {a} fit inside.',
    'teach.gapFamily': 'You know {small} × {large} = {product}, so the missing number is {b}.',
  ...
  nb: {
    ...
    'tier.math.19.name': 'Enere og tiere',
    'tier.math.19.blurb': 'Ingenting endrer seg, eller det kommer en null bakerst.',
    'tier.math.20.name': 'Toere',
    'tier.math.20.blurb': 'Å doble, som du alt kan fra pluss.',
    'tier.math.21.name': 'Femmere',
    'tier.math.21.blurb': 'Halvparten av ti, hver eneste gang.',
    'tier.math.22.name': 'Kvadrattall',
    'tier.math.22.blurb': 'Et tall ganget med seg selv: tre treere, fire firere.',
    'tier.math.23.name': 'Treere og firere',
    'tier.math.23.blurb': 'Tell i treere, eller dobbel og dobbel en gang til.',
    'tier.math.24.name': 'De seks vanskelige',
    'tier.math.24.blurb': 'Seks, sju, åtte og ni — de siste som står igjen.',
    'tier.math.25.name': 'Enere og tiere som mangler',
    'tier.math.25.blurb': 'De samme to reglene, spurt andre veien.',
    'tier.math.26.name': 'Toere som mangler',
    'tier.math.26.blurb': 'Hvor mange toere skal det til for å komme dit?',
    'tier.math.27.name': 'Femmere som mangler',
    'tier.math.27.blurb': 'Tell oppover i femmere til du lander på tallet.',
    'tier.math.28.name': 'Kvadrattall som mangler',
    'tier.math.28.blurb': 'Hvilket tall ganget med seg selv blir dette?',
    'tier.math.29.name': 'Treere og firere som mangler',
    'tier.math.29.blurb': 'Du vet svaret — finn tallet som laget det.',
    'tier.math.30.name': 'De seks vanskelige, baklengs',
    'tier.math.30.blurb': 'De seks du syntes var verst, jaktet på baklengs.',
    'group.plus': 'Pluss',
    'group.minus': 'Minus',
    'group.tens': 'Tiere og enere',
    'group.column': 'Oppstilling',
    'group.times': 'Ganging',
    'tier.math.31.name': 'Gange med tiere',
    'tier.math.31.blurb': 'Sifrene står stille, og det kommer en null bakerst.',
    'tier.math.32.name': 'Oppstilling uten mente',
    'tier.math.32.blurb': 'Sett dem under hverandre og gang hver kolonne for seg.',
    'tier.math.33.name': 'Oppstilling med mente',
    'tier.math.33.blurb': 'Gang først, og legg så til det du hadde i mente.',
    'tier.math.34.name': 'Tresifret oppstilling',
    'tier.math.34.blurb': 'Mente to ganger, hele veien bortover.',
    'tier.math.35.name': 'To rader å legge sammen',
    'tier.math.35.blurb': 'Gang med enerne, så med tierne, og legg sammen.',
    'tier.math.36.name': 'Tre sifre ganger to',
    'tier.math.36.blurb': 'Hele metoden, fra ende til annen.',
    'group.gap': 'Tallet som mangler',
    'group.colmul': 'Ganging med oppstilling',
    'skill.tensx': 'Gange med tiere',
    'skill.colx21': 'Oppstilt ×',
    'skill.colx21c': 'Oppstilt × mente',
    'skill.colx31c': 'Tresifret ×',
    'skill.colx22': 'To rader',
    'skill.colx32': 'Tre ganger to',
    'sign.divide': ':',
    'tier.math.37.name': 'Dele på én og ti',
    'tier.math.37.blurb': 'Ingenting endrer seg, eller en null faller av.',
    'tier.math.38.name': 'Dele i to',
    'tier.math.38.blurb': 'Halvere, som du alt kan fra doblingen.',
    'tier.math.39.name': 'Dele på fem',
    'tier.math.39.blurb': 'Halvparten av ti, talt tilbake i femmere.',
    'tier.math.40.name': 'Kvadrattallene delt tilbake',
    'tier.math.40.blurb': 'Førtini delt i sjuere, og resten av dem.',
    'tier.math.41.name': 'Dele på tre og fire',
    'tier.math.41.blurb': 'Tabellene du bygde opp i tre- og firere.',
    'tier.math.42.name': 'De siste seks',
    'tier.math.42.blurb': 'De alle barn synes er vanskelige.',
    'tier.math.43.name': 'Dele på hele tiere',
    'tier.math.43.blurb': 'Sifrene blir stående, og en null faller av.',
    'tier.math.44.name': 'Oppstilt divisjon',
    'tier.math.44.blurb': 'Ett siffer om gangen, fra den største enden.',
    'tier.math.45.name': 'Ta resten med ned',
    'tier.math.45.blurb': 'Det som blir igjen, blir med til neste siffer.',
    'tier.math.46.name': 'Når første siffer er for lite',
    'tier.math.46.blurb': 'Ta to sifre sammen, så blir svaret kortere.',
    'tier.math.47.name': 'Tre sifre, delt ut',
    'tier.math.47.blurb': 'De samme tre stegene, en gang til.',
    'tier.math.48.name': 'En null i svaret',
    'tier.math.48.blurb': 'Steget blir til ingenting, og ingentinget skrives likevel.',
    'tier.math.49.name': 'Noe til overs',
    'tier.math.49.blurb': 'Del videre til det ikke kommer ned mer.',
    'tier.math.50.name': 'Hele metoden',
    'tier.math.50.blurb': 'Tre sifre, og det som blir til overs til slutt.',
    'group.divfacts': 'Deling',
    'group.coldiv': 'Oppstilt divisjon',
    'skill.div10': 'Dele på hele tiere',
    'skill.div21': 'Oppstilt :',
    'skill.div21x': 'Ta resten med ned',
    'skill.div21s': 'Kortere svar',
    'skill.div31': 'Tresifret :',
    'skill.div31z': 'En null i svaret',
    'skill.div21r': 'Noe til overs',
    'skill.div31r': 'Tre sifre med rest',
    'teach.divGaveDividend': 'Det er tallet du skulle dele ut.',
    'teach.divGaveDivisor': 'Det er hvor mange du skulle dele det på.',
    'teach.divTookAway': 'Ikke trekke fra — hvor mange av dem som får plass.',
    'teach.divMultiplied': 'Det er dem ganget, ikke delt ut.',
    'teach.divNeighbour': 'Det er i riktig tabell, men et annet av dem.',
    'teach.divPlain': '{b} går {answer} ganger opp i {a}.',
    'teach.divFamily': 'Du kan {small} × {large} = {a}, så {a} : {b} = {answer}.',
    'teach.divForgotBringDown': 'Ta neste siffer ned ved siden av det som er igjen.',
    'teach.divIgnoredRemainder': 'Noe ble til overs, og det forsvinner ikke av seg selv.',
    'teach.divZeroStep': 'Det går ikke opp der i det hele tatt — skriv en null og gå videre.',
    'teach.divDroppedRemainder': 'Det som blir til overs, går ned til neste siffer.',
    'teach.divBackwards': 'Tallet øverst er det som skal deles ut, hver gang.',
    'teach.divMulInstead': 'Det er dem ganget, og dette skal deles ut.',
    'teach.divSubInstead': 'Det er dem trukket fra, og dette skal deles ut.',
    'teach.divColPlain': '{a} delt på {b} er {quotient}.',
    'teach.divColRest': '{a} delt på {b} er {quotient}, og {rest} til overs.',
    'prompt.divEgg': 'Et kaldt egg! Still opp og del:',
    'prompt.divEgg1': 'Egget rører på seg! Steg for steg:',
    'prompt.divEgg2': 'Det slår sprekker! Ett stykke til å dele:',
    'prompt.divForgot': '{name} har glemt matpakka. Del ut:',
    'prompt.divHungry': '{name} er sulten! Maten er dette stykket:',
    'prompt.divSnack': '{name} vil ha litt mat. Del ut:',
    'teach.mulColForgotShift': 'Den raden er tierne, så den trenger nullen bakerst.',
    'teach.mulColCarriedFirst': 'Gang sifferet først, og legg mente til det du får.',
    'teach.mulColFullProduct': 'En kolonne rommer bare ett siffer — resten går i mente.',
    'teach.mulColForgotCarry': 'Det som gikk over tieren må flyttes til naboen.',
    'teach.mulColOnlyOnes': 'Det er bare den første raden — tierne står fortsatt igjen.',
    'teach.mulColAddedInstead': 'Det er dem lagt sammen, ikke ganget.',
    'teach.mulColPlain': '{a} ganger {b} blir {total}.',
    ...
    'prompt.gapEgg': 'Et kaldt egg! Hvilket tall mangler?',
    'prompt.gapEgg1': 'Egget rører på seg! Finn tallet som mangler:',
    'prompt.gapEgg2': 'Det slår sprekker! Ett tall til som mangler:',
    'prompt.gapForgot': '{name} har glemt matbiten sin. Hvilket tall mangler?',
    'prompt.gapHungry': '{name} er sulten! Finn tallet som mangler:',
    'prompt.gapSnack': '{name} har lyst på en matbit. Hvilket tall mangler?',
    'teach.mulGaveSum': 'Det er dem lagt sammen, ikke den ene mange ganger.',
    'teach.mulGaveFactor': 'Det er bare det ene tallet.',
    'teach.mulOffByOneRow': 'Én rad for mye, eller én for lite — tell radene om igjen.',
    'teach.mulNeighbour': 'Det er i riktig gangetabell, men det er et annet svar i den.',
    'teach.mulPlain': '{a} ganger {b} blir {product}.',
    'teach.gapGaveProduct': 'Det er dit du skal, ikke tallet som tar deg dit.',
    'teach.gapGaveFactor': 'Det tallet hadde du fra før.',
    'teach.gapTookAway': 'Det er å trekke fra — her spør vi hvor mange {a}-ere som får plass.',
    'teach.gapFamily': 'Du kan {small} × {large} = {product}, så tallet som mangler er {b}.',
