...
- [ ] Pricing page
- [ ] Docs shell
`,M=["id","title","goal","project","people","due","estimate","created","done","working","blocked-by","part-of","x"],L=new Set(["project","people","blocked-by","part-of"]),Y=/^\s*[-*]\s+\[([ xX])\]\s?(.*)$/;function N(e){const n=e.trim();return n.length>=2&&(n[0]==='"'||n[0]==="'")&&n[n.length-1]===n[0]?n.slice(1,-1):n}function B(e){const n=e.trim();return n===""?"":n==="true"?!0:n==="false"?!1:n==="null"||n==="~"?null:N(n)}function W(e){const n=e.trim().slice(1,-1).trim();if(n==="")return[];const t=[];let s="",i=null;for(const o of n)i?(o===i&&(i=null),s+=o):o==='"'||o==="'"?(i=o,s+=o):o===","?(t.push(s),s=""):s+=o;return t.push(s),t.map(o=>N(o)).filter(o=>o!=="")}function x(e){const n=String(e).replace(/\r\n/g,`
`);if(!n.startsWith(`---
`))return{data:{},body:n.replace(/^\n+/,"")};const t=n.indexOf(`
---`,3);if(t===-1)return{data:{},body:n};const s=n.slice(4,t+1),i=n.indexOf(`
`,t+1),o=i===-1?"":n.slice(i+1),r={},a=s.split(`
`);let l=null;for(const c of a){if(c.trim()===""||c.trimStart().startsWith("#"))continue;const u=/^\s*-\s+(.*)$/.exec(c);if(u&&l){r[l].push(B(u[1]));continue}const g=/^([A-Za-z0-9_][A-Za-z0-9_-]*)\s*:\s*(.*)$/.exec(c);if(!g)continue;const[,p,h]=g;l=null,h.trim()===""?(r[p]=[],l=p):h.trim().startsWith("[")&&h.trim().endsWith("]")?r[p]=W(h):r[p]=B(h)}for(const[c,u]of Object.entries(r))Array.isArray(u)&&u.length===0&&!L.has(c)&&(r[c]="");return{data:r,body:o}}function G(e){const n=String(e);return n===""||/^[#&*!|>%@`?-]/.test(n)||/[:,[\]{}]/.test(n)||n!==n.trim()||["true","false","null","~"].includes(n)}function D(e){return typeof e=="boolean"||typeof e=="number"?String(e):e==null?"":G(e)?`'${String(e).replace(/'/g,"''")}'`:String(e)}function P(e,n){return Array.isArray(n)||L.has(e)?`[${(Array.isArray(n)?n:[n].filter(s=>s!==""&&s!=null)).map(D).join(", ")}]`:D(n)}function v(e,n=""){const s=[...M.filter(o=>o in e),...Object.keys(e).filter(o=>!M.includes(o))].map(o=>`${o}: ${P(o,e[o])}`),i=String(n).replace(/\s+$/,"");return`---
${s.join(`
`)}
---
${i?`${i}
...
Reply with JSON: {"estimate": "3d", "why": "..."}`}]},parse(e){const n=T(e),t=b(typeof n=="string"?n:(n==null?void 0:n.estimate)??(n==null?void 0:n.duration)),s=/(\d+(?:\.\d+)?)\s*([hdw])/i.exec(t);if(!s){const o=new Error(`Model returned an unusable estimate: "${t||"(empty)"}"`);throw o.raw=JSON.stringify(n),o}const i=`${Number(s[1])}${s[2].toLowerCase()}`;return[{kind:"estimate",label:i,detail:b((n==null?void 0:n.why)??(n==null?void 0:n.rationale)),estimate:i}]}},fn={subtasks:xe,missing:ve,estimate:$e};export{ee as $,ln as A,ue as B,He as C,Je as D,F as E,fn as F,Ke as G,un as H,$ as I,le as J,nn as K,rn as L,an as M,de as N,ce as O,S as P,dn as Q,tn as R,ze as S,on as T,sn as U,Xe as V,x as W,v as X,V as Y,X as Z,qe as _,Pe as a,ne as a0,q as a1,J as a2,H as a3,Q as a4,j as a5,pe as a6,oe as a7,se as a8,te as a9,T as aa,we as ab,Ge as b,We as c,Ye as d,Ue as e,je as f,Ie as g,Re as h,en as i,Fe as j,Ne as k,Le as l,Ee as m,De as n,Be as o,Me as p,Ce as q,Ae as r,Te as s,Oe as t,Ze as u,cn as v,Qe as w,Ve as x,m as y,fe as z};
